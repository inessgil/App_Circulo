...
public class AddImageInteractor extends BaseUseCase<Void> implements Interactor<Map<String, String>, Void> {
    ...
    CirculoRepository.AddImageCallback dataCallback = new CirculoRepository.AddImageCallback() {
        @Override
    ...
    @Override
    public void run() {
        repository.addImage(input.get("date"), input.get("name"), input.get("topic"), input.get("content"), dataCallback);
    }

    @Override
    public <R extends DefaultCallback<Void>> void execute(Map<String, String> input, R defaultCallback) {
        this.callback = ((AddImageCallback) defaultCallback);
        this.input = input;
